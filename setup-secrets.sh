#!/bin/bash

# Script para configurar secrets do GitHub via CLI
# Baseado no arquivo .env do projeto

echo "🔧 Configurando secrets do GitHub Actions..."
echo "📍 Repositório: tgferrmonitor/monitor"
echo ""

# Verificar se gh CLI está instalado
if ! command -v gh &> /dev/null; then
    echo "❌ GitHub CLI não encontrado!"
    echo "💡 Para instalar:"
    echo "   sudo apt install gh"
    echo "   ou"  
    echo "   sudo snap install gh"
    exit 1
fi

# Verificar se está autenticado
if ! gh auth status &> /dev/null; then
    echo "❌ Não autenticado no GitHub!"
    echo "💡 Execute: gh auth login"
    exit 1
fi

echo "🔐 Configurando secrets..."

# Roblox
echo "  🎮 ROBLOSECURITY..."
gh secret set ROBLOSECURITY --body "_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_CAEaAhAB."

echo "  👥 PLAYERS..."
gh secret set PLAYERS --body "7738147772,8631412913,9005903775,8419659295,2616521101"

# AWS S3 / Magalu Cloud
echo "  ☁️  S3_BUCKET..."
gh secret set S3_BUCKET --body "bucket-tgferr-monitor"

echo "  🌐 S3_ENDPOINT..."
gh secret set S3_ENDPOINT --body "https://bucket-tgferr-monitor.br-se1.magaluobjects.com"

echo "  🔑 S3_KEY..."
gh secret set S3_KEY --body "9c389761-baed-4495-b5ab-ce287e3fbcb4"

echo "  🔐 S3_SECRET..."
gh secret set S3_SECRET --body "fec244d0-dfd1-46fe-9123-c574f2f576e1"

echo "  🌍 S3_REGION..."
gh secret set S3_REGION --body "br-se1"

# Email
echo "  📧 EMAIL_USER..."
gh secret set EMAIL_USER --body "tiagomoz1988@gmail.com"

echo "  🔒 EMAIL_PASS..."
gh secret set EMAIL_PASS --body "juudiidzyqbcfvtz"

echo "  📨 EMAIL_TO..."
gh secret set EMAIL_TO --body "tiagomoz1988@gmail.com"

# Timezone
echo "  🕐 TZ_OFFSET_MINUTES..."
gh secret set TZ_OFFSET_MINUTES --body "-180"

echo ""
echo "✅ Todos os secrets foram configurados com sucesso!"
echo ""
echo "📋 Verificando secrets criados..."
gh secret list

echo ""
echo "🚀 Pronto! O GitHub Actions deve funcionar na próxima execução."
echo "⏰ O workflow roda a cada 5 minutos automaticamente."
echo ""
echo "🔍 Para monitorar:"
echo "   📊 GitHub Actions: https://github.com/tgferrmonitor/monitor/actions"
echo "   🌐 Interface Web: https://bucket-tgferr-monitor.br-se1.magaluobjects.com/"
echo "   📁 Arquivos S3: https://bucket-tgferr-monitor.br-se1.magaluobjects.com/080925.json"
