...
async function getJsonFromS3(key) {
  try {
    ...
    // If object not found, return null. Otherwise return null to tolerate missing file
    return null;
  }
}
...
async function updateDailyHours(presenceData, minutesIncrement = 5) {
  try {
    ...
    // Prefix configurável para relatórios de horas (padrão 'reports/hours')
    const hoursPrefix = process.env.S3_HOURS_PREFIX || "reports/hours";
    const key = `${hoursPrefix}/${filename}`;
